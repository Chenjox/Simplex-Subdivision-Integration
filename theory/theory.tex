...
\tikzset{
    scale basis/.style={x={#1*1.0cm},y={#1*1.0cm}}
}
...
\subsubsection{Integration Rules in~\cite{lynessModerateDegreeSymmetric1975}}

The Integration Rules in~\cite{lynessModerateDegreeSymmetric1975} which satisfy the \texttt{PI} and $v = \overline{v}$ constraint.
This only allows rules, which only allow positive weight where all points lie inside the triangle.\marginnote{Some Values were hard to read in my print of the paper.} 
...
\begin{table}[H]
    ...
    \begin{tabular}{c S[table-format=1.17] S[table-format=1.17] S[table-format=1.17] S[table-format=1.17] c}
        ...
        \hline
       4 & 0.3298552309659655 & 0.8168475729804585 & 0.09157621350977073 & {$1 - \xi_1 - \xi_2$} & 3 \\
       4 & 0.6701447690340345 & 0.1081030181680702 & 0.44594849091596490 & {$1 - \xi_1 - \xi_2$} & 3 \\
       \hline
       6 & 0.3503588271790222 & 0.5014265096581342 & 0.2492867451709329  & {$1 - \xi_1 - \xi_2$} & 3 \\ 
...
\begin{table}[H]
    ...
    \begin{tabular}{c S[table-format=1.17] S[table-format=1.17] S[table-format=1.17] S[table-format=1.17] c}
        ...
        Deg & {Weight $w_i$} & {$\xi_1$} & {$\xi_2$} & {$\xi_3$} & Mult\\
        \hline
       1 & 1.0 & 0.333333333333333 & 0.333333333333333 & {$1 - \xi_1 - \xi_2$} & 1 \\
       \hline
       2 & 0.333333333333333 & 0.666666666666667 & 0.166666666666667 & {$1 - \xi_1 - \xi_2$} & 3 \\ 
       \hline
       3 & -0.562500000000000 & 0.333333333333333 & 0.333333333333333 & {$1 - \xi_1 - \xi_2$} & 1 \\
       3 & 0.520833333333333 & 0.6 & 0.2 & {$1 - \xi_1 - \xi_2$} & 3 \\
       \hline
       4 & 0.223381589678011 & 0.108103018168070 & 0.445948490915965 & {$1 - \xi_1 - \xi_2$} & 3 \\
       4 & 0.109951743655322 & 0.816847572980459 & 0,091576213509771 & {$1 - \xi_1 - \xi_2$} & 3 \\
       \hline
       5 & 0.225000000000000 & 0.333333333333333 & 0.333333333333333 & {$1 - \xi_1 - \xi_2$} & 1 \\
...
\begin{Figure}
    ...
    \begin{tikzpicture}
        \begin{scope}[shift={(0,0)}]
            \input{figures/Dunavant2D-1.tikz}
        \end{scope}
        \begin{scope}[shift={(2,0)}]
            \input{figures/Dunavant2D-2.tikz}
        \end{scope}
        \begin{scope}[shift={(4,0)}]
            \input{figures/Dunavant2D-3.tikz}
        \end{scope}
    \end{tikzpicture}
\end{Figure}
...
\subsubsection{Edgewise Simplex Subdivision}

The edgewise simplex subdivision can also be applied to uniformly subdivide any $d$-simplex.
The points inside the $r$-th edgewise subdivision are indexed by $d+1$ integers $i_1, i_2, \cdots, i_{d+1}$, which satisfy following 
relation \begin{equation}
    i_1 + i_2 + i_3 + \cdots + i_{d+1} = r
\end{equation}
...
To count the number $s_j$ of $j$-simplizes the counting formula from~\cite{edelsbrunnerEdgewiseSubdivisionSimplex2000} : \begin{align*}
    C_m &= \binom{d+ m \cdot r}{d} \\
    s_j &= \sum_{i=0}^j {(-1)}^i \cdot \binom{j}{i} \cdot C_{j+1-i}
\end{align*}
...
\begin{Figure}
    \centering
    \begin{tikzpicture}
        \draw[gray,thick] (0,0) coordinate (ca) 
          -- (-60:9cm) coordinate (cb) 
          -- (-120:9cm) coordinate (cc) -- cycle;
        \newcommand{\indcoord}[3]{
            \draw[fill] (barycentric cs:ca=#1/4,cb=#2/4,cc=#3/4) coordinate (cb#1#2#3) circle (2pt) node[above] {$(#1,#2,#3)$};
        }
        \indcoord{4}{0}{0}
        \indcoord{3}{1}{0}
        \indcoord{2}{2}{0}
...
\begin{Figure}
    \centering
    \begin{tikzpicture}
        \path[gray,thick] (0,0) coordinate (ca) 
          -- (-60:6cm) coordinate (cb) 
          -- (-120:6cm) coordinate (cc) -- cycle;
        \newcommand{\indcoord}[3]{
            \draw[fill] (barycentric cs:ca=#1/3,cb=#2/3,cc=#3/3) coordinate (cb#1#2#3) circle (2pt);% node[above] {$(#1,#2,#3)$};
        }
        %\indcoord{3}{0}{0}
        ...
        \draw[shorten >= 3pt,shorten <= 3pt,->] (cb111) -- (cb210) node[midway,right] {$(1,0,-1)$};
        \draw[shorten >= 3pt,shorten <= 3pt,->] (cb111) -- (cb120) node[midway,below] {$(0,1,-1)$};
        \draw[shorten >= 3pt,shorten <= 3pt,->] (cb111) -- (cb021) node[midway,right] {$(-1,1,0)$};
        \draw[shorten >= 3pt,shorten <= 3pt,->] (cb111) -- (cb012) node[midway,left] {$(-1,0,1)$};
        \draw[shorten >= 3pt,shorten <= 3pt,->] (cb111) -- (cb102) node[midway,above] {$(0,-1,1)$};
        \draw[shorten >= 3pt,shorten <= 3pt,->] (cb111) -- (cb201) node[midway,left] {$(1,-1,0)$};
    \end{tikzpicture}
    \caption{Traversal through the points}
    \label{fig:4traversal}
\end{Figure}

The integration domains will be identified by their \emph{first-most} point $I_0 = (i_1,i_2,i_3)$.
In order to calculate the neighboring points the following formulas will be used: \begin{align}
    I_1 &= (i_1,i_2,i_3) + (0,-1,1) & I_2 &= (i_1,i_2,i_3) + (-1,0,1) & I_3 &= (i_1,i_2,i_3) + (-1,1,0)
\end{align}

The \emph{first-most} points will be enumerated by iterating through the 2 first most indeces.

\begin{Figure}
    \centering
    \begin{tikzpicture}
        \begin{scope}[shift={(0,0)},scale basis=4]
            \input{figures/EdgeSubdivision2D-1.tikz}
        \end{scope}
        \begin{scope}[shift={(4.5,0)},scale basis=4]
            \input{figures/EdgeSubdivision2D-2.tikz}
        \end{scope}
        \begin{scope}[shift={(9,0)},scale basis=4]
            \input{figures/EdgeSubdivision2D-3.tikz}
        \end{scope}
        \begin{scope}[shift={(0,-4.5)},scale basis=4]
            \input{figures/EdgeSubdivision2D-4.tikz}
        \end{scope}
        \begin{scope}[shift={(4.5,-4.5)},scale basis=4]
            \input{figures/EdgeSubdivision2D-5.tikz}
        \end{scope}
        \begin{scope}[shift={(9,-4.5)},scale basis=4]
            \input{figures/EdgeSubdivision2D-6.tikz}
        \end{scope}
    \end{tikzpicture}
\end{Figure}
...
\subsubsection{Hierarchic Subdivision}
...
\subsubsection{Edgewise Subdivision}

\begin{Figure}
    \centering
    \begin{tikzpicture}[engineering axonometry]
        \begin{scope}[scale=7.0]
            \coordinate (b1) at ({sqrt(8/9)},0,0);
            \coordinate (b2) at ({-sqrt(2/9)},{sqrt(2/3)},0);
            \coordinate (b3) at ({-sqrt(2/9)},{-sqrt(2/3)},0);
            \coordinate (b4) at (0,0,4/3);
        \end{scope}
        \newcommand{\indcoord}[4]{
            \draw[fill] (barycentric cs:b1=#1/4,b2=#2/4,b3=#3/4,b4=#4/4) coordinate (cb#1#2#3) circle (2pt) node[font=\tiny,right] {$(#1,#2,#3,#4)$};
        }
        \indcoord{0}{0}{0}{4}
        \indcoord{0}{0}{1}{3}
        \indcoord{0}{0}{2}{2}
